...
describe("INTERNAL -- requestHTTP", () =>
{
	...
	it("rejects an erroneous url", async () =>
	{
		...
		try
		{
			await requestHTTP("/simple/404.html", defaultAuth, GET, cache, parsedOptions());
		}
		catch (error)
		{
			expect(error)
	...
	describe("caching", () =>
	{
		...
		it("stores the response of redirected urls", async () =>
		{
			...
			requestHTTP(url1, defaultAuth, GET, cache, options);
		...
		it("does not store the error from an erroneous url", async () =>
		{
			...
			try
			{
				await requestHTTP("/simple/404.html", defaultAuth, GET, cache, options);
			}
			catch
			{
				errorWasThrown = true;
	...
	describe("options", () =>
	{
		it("retryHeadFail = false", async () =>
		{
			...
			const {response: {status}} = await requestHTTP(url, defaultAuth, HEAD, cache, options);
		...
		it("retryHeadFail = false (#2)", async () =>
		{
			...
			const {response: {status}} = await requestHTTP(url, defaultAuth, GET, cache, options);
